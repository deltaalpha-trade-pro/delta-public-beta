...
def request(api_base: str, method: str, path: str, token: str | None = None, payload: object | None = None) -> int:
...
def main() -> int:
    ...
    intelligence.add_argument("input", help="Text or JSON-compatible string for the intelligence request")
    ...
    if args.command == "register":
        ...
        # The public API acknowledges node registration without exposing the phone inbound.
        return request(args.api, "POST", "/nodes/register", args.token)
    if args.command == "intelligence":
        if not args.token:
            parser.error("intelligence requires --token or WHALEZ_API_KEY")
        payload = {"input": args.input}
        return request(args.api, "POST", "/intelligence", args.token, payload)
