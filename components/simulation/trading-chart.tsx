...
import { Play, Pause, RotateCcw, TrendingUp, TrendingDown, Info } from "lucide-react"
...
export function TradingChart({ sessionId, onSessionCreate }: TradingChartProps) {
  ...
  const calculateRSI = useCallback((candleData: CandleData[], period = 14): number => {
    if (candleData.length < period + 1) return 50

    const changes = candleData.slice(-period - 1).map((c, i, arr) => {
      if (i === 0) return 0
  ...
  useEffect(() => {
    ...
    intervalRef.current = setInterval(() => {
      setCandles((prev) => {
        ...
        const newRsi = calculateRSI(updatedCandles)
        setRsi(newRsi)
  ...
  }, [isRunning, candles.length, generateCandle, calculateRSI])
  ...
  // AI Narration helper
  const getAINarration = useCallback((): string => {
    if (rsi > 70) {
      return "RSI indicates overbought conditions. Price may face resistance or pullback."
    } else if (rsi < 30) {
      return "RSI indicates oversold conditions. Potential support zone or bounce opportunity."
    } else if (priceChange > 0) {
      return "Upward momentum detected. Bullish pressure building in current timeframe."
    } else if (priceChange < 0) {
      return "Downward pressure observed. Bearish sentiment in recent candles."
    }
    return "Price action is neutral. Monitor key support and resistance levels."
  }, [rsi, priceChange])
    ...
    <Card className="bg-card border-border">
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <CardTitle className="text-sm font-medium text-foreground">FX Simulation Chart</CardTitle>
          ...
          <div className="flex items-center gap-2">
            <Select value={timeframe} onValueChange={(v) => setTimeframe(v as Timeframe)}>
              <SelectTrigger className="w-16 h-7 text-xs bg-transparent">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {TIMEFRAMES.map((tf) => (
            ...
            <Select value={asset} onValueChange={(v) => handleAssetChange(v as SimulationAsset)}>
              <SelectTrigger className="w-28 h-7 text-xs bg-transparent">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="flex items-center gap-3 mt-2">
            ...
            {showIndicators && (
              <div className="flex items-center gap-2 text-xs">
                ...
                <span
                  className={`font-mono font-medium ${rsi > 70 ? "text-red-500" : rsi < 30 ? "text-green-500" : "text-foreground"}`}
                >
                  {rsi.toFixed(1)}
                </span>
              </div>
      ...
      <CardContent className="pt-0">
        ...
        {isRunning && showIndicators && (
          <div className="mt-3 p-2 rounded-lg bg-primary/5 border border-primary/10">
            <div className="flex items-start gap-2">
              ...
              <p className="text-xs text-muted-foreground">{getAINarration()}</p>
            </div>
          </div>
        )}
        ...
        <p className="text-xs text-muted-foreground mt-3">
          Educational simulation only. No live trading. No broker APIs. All data is synthetic and generated locally for
